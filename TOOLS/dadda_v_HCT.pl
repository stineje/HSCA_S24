...
# perl script to generate a truncated Dadda tree multiplier (hybrid correction)
...
# -----------------------------------------------------------
if(length($MODULE)==0){
    printf ("module HCTdadda%s_%s_%s_%s (Z, X, Y);\n", $XBITS, $YBITS, $ZBITS, $K);
}
else{
...
for($y=0;$y<$YBITS;$y++) {
    for($x=0;$x<$XBITS;$x++) {
        if($x+$y>=$XBITS+$YBITS-$ZBITS-$K){
            if(@carray[$x+$y]==1){
                @harray[$x+$y]=$y+1 if($y+1>@harray[$x+$y]);
            }
            else{
...
# Add the bits of the variable correction
@harray[$XBITS+$YBITS-$ZBITS-$K] += $nbitsvar;
...
for($y=0;$y<$YBITS;$y++) {
    for($x=0;$x<$XBITS-1;$x++) {
        if($x+$y==$XBITS+$YBITS-$ZBITS-$K-1 && $nvar<$nbitsvar){
            if($var1st==1){
                $var1st=0;
            }
            else{
...
# Add the bits for the constant correction (+ rounding)
for($x=$XBITS+$YBITS-$ZBITS-$K;$x<$XBITS+$YBITS-1;$x++){
    if(@carray[$x]==1){
